...
import { stringify, parse, SxpbLone, SxpbMany } from "../src/index.js";
...
describe("Roundtrip", () => {
  it("handles complex nested structure", () => {
    ...
    // `stringify` serializes it as array body with messages.
    // `parse` parses it back as array of plain objects.

    // `scripts` is SxpbMany.
    // `stringify` serializes it as `(scripts (()) (build tsc) (test vitest))`.
    // `parse` parses `(scripts (()) ...)` and sees fields `(build tsc)`.
    // It detects `manyof_body` and returns `SxpbMany`.

    // `config` is SxpbLone.
    // `stringify` -> `((config debug) +true)`.
    // `parse` -> `SxpbLone`.
  ...
  it("handles mixed arrays", () => {
    ...
    // This parser/serializer handles what it can.
